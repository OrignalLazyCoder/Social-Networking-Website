...
<?php
  ...
  if($row["Sender"]==$_COOKIE["user_first_name"]){
    if($get_profile["Profile_Picture"]==""){
       echo "<div class='sender'><span class='original_sender'><div class='shape_sender'>".$row["Message"]."</div></span><img class='general_profile_sender' src='../Profile_Pictures/default.png' height='35' width='35' title='".$row["Sender"]."'/>";
       echo "<br>".$row["Sender"]."</div>" ;
    }else{
        $username_picture = $get_profile["Profile_Picture"];
        echo "<div class='sender'><span class='original_sender'><div class='shape_sender'>".$row["Message"]."</div></span><img class='general_profile_sender' src='../Profile_Pictures/$username_picture' height='35' width='35' title='".$row["Sender"]."'/>";
        echo "<br>".$row["Sender"]."</div>" ;
    }
 	...
 	if($get_profile["Profile_Picture"]==""){
       echo "<div class='receiver'><img class='general_profile_receiver' src='../Profile_Pictures/default.png' height='35' width='35' title='".$row["Sender"]."'/><span class='original_receiver'><div class='shape_receiver'>".$row["Message"]."</div></span></div>";
        ...
        $username_picture = $get_profile["Profile_Picture"];
        echo "<div class='receiver'><img class='general_profile_receiver' src='../Profile_Pictures/$username_picture' height='35' width='35' title='".$row["Sender"]."'/><span class='original_receiver'><div class='shape_receiver'>".$row["Message"]."</div></span>.<br></div>";
        //setcookie("last_username",$row["Sender"],time() + (86400 * 30));
				//echo "<script>$('#get_chat_logs').stop().animate({scrollTop:$('#get_chat_logs')[0].scrollHeight},800);</script>";
    }
